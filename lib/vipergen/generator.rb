module Vipergen
	...
	class Generator
		...
		def self.rename_file_content(filename, name, author)
			...
			content = content.gsub((Vipergen::Generator::LOWER_REPLACEMENT_KEY), name[0, 1].downcase + name[1..-1])
			...
			content = content.gsub((Vipergen::Generator::YEAR_REPLACEMENT_KEY), "#{Time.now.year}")
